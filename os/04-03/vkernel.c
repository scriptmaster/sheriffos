...
VV_LOCAL_SYMBOL  int compare_8496757889636869320_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VV_LOCAL_SYMBOL void main__kernel(void);
...
u16* _const_main__cga_buffer; // inited later
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	if (sidx == _IError_None___index) return 65612;
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), /*115 &string*/0xfe10, {.d_s = _SLIT("a8e0ced")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
string string_replace_each(string s, Array_string vals) {
	...
	qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_8496757889636869320_RepIndex_by_idx);
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	_const_main__cga_buffer = ((u16*)(0xb8000));
}
void _vcleanup(void) {
}
